/***************************************************************************************************
 * File:         $URL: http://obtuse.cs.jhu.edu/Projects/Library/Source/Branches/NASABot/Src/Text/StringImpl.hpp $
...
//----------------------------------------------------------------------------------------------------
template<class CharType>
BOOL TString<CharType>::operator== (CONST CharType* pkData) CONST
{
  SIZE_T Length = CharTraits<CharType>::Length(pkData);
  if (Length != m_Length)
    return false;
  int c = CharTraits<CharType>::StrCmp(ConstStr(), pkData, Length); 
  return (c == 0);
}
//----------------------------------------------------------------------------------------------------
template<class CharType>
...
//----------------------------------------------------------------------------------------------------
template<class CharType>
Array< TString<CharType> > TString<CharType>::Split(CharType Sep)
{
  Array<String> S;
  SIZE_T Start = 0;
  SIZE_T i = 0;
...
VOID TString<CharType>::TrimLeft()
{
  ...
  memmove(m_Str, m_Str+i, m_Length-i);
  m_Length -= i;
...
Array< TString<CharType> > TString<CharType>::Split(Predicate P)
{
  Array<String> S;
  SIZE_T Start = 0;
  SIZE_T i = 0;
