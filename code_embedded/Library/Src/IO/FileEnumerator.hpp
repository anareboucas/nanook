/***************************************************************************************************
 * File:         $URL: http://obtuse.cs.jhu.edu/Projects/Library/Source/Branches/NASABot/Src/IO/FileEnumerator.hpp $
...
namespace Bolitho
{
  namespace IO
  {
    ...
    { enum { RECURSIVE = 1, HIDDEN = 2, DEFAULT = 0 }; };

    class LIBRARY_API FileEnumerator : public Object
    {
      DeclareRuntimeType();
    public:
      FileEnumerator( LPCTSTR Path = TEXT("."), LPCTSTR SearchPattern = TEXT(".*"), DWORD Flags = FileEnumeratorFlags::DEFAULT );
      virtual ~FileEnumerator();
      
      BOOL HasMoreElements() CONST;
      CONST FileInfo& CurrentElement() CONST;
      VOID NextElement();
