/***************************************************************************************************
 * File:         $URL: http://obtuse.cs.jhu.edu/Projects/Library/Source/Branches/NASABot/Src/Math/SparseMatrixImpl.hpp $
 * Author:       $Author: OBTUSE\matthew $
 * Revision:     $Rev: 1282 $
 * Last Updated: $Date: 2007-11-18 16:53:20 -0500 (Sun, 18 Nov 2007) $
 * 
 * 
...
template<class T>
template<class U>
VOID SparseMatrix<T>::Multiply( CONST Vector<U>& In,Vector<U>& Out) CONST
{
	for (UINT i=0; i<m_Rows; i++)
  {
		U Temp = U();
...
template<class T>
template<class U>
VOID SparseMatrix<T>::Multiply(System::ThreadPool& Pool, CONST Vector<U>& In, Vector<U>& Out) CONST
{
  ...
  if (m_Rows > BlockCount * 64)
  {
    ...
    while (Offset < m_Rows)
    {
      ...
      Pool.Enqueue((LPTHREAD_START_ROUTINE)&MultiplyThreadProc<T,U>, pInfo);
...
template<class T>
template<class U>
VOID SparseMatrix<T>::SolveJacobi(CONST SparseMatrix<T>& M, CONST Vector<typename SparseMatrix<U>::MATRIXENTRY>& Diagonal, CONST Vector<U>& b, UINT SolverIterations, Vector<U>& Solution, BOOL Reset)
{
	Vector<U> Md;
...
template<class T>
template<class U>
VOID SparseMatrix<T>::SolveJacobi(System::ThreadPool& Pool, CONST SparseMatrix<T>& M, CONST Vector<typename SparseMatrix<U>::MATRIXENTRY>& Diagonal, CONST Vector<U>& b, UINT SolverIterations, Vector<U>& Solution, BOOL Reset)
{
	Vector<U> Md;
