/***************************************************************************************************
 * File:         $URL: http://obtuse.cs.jhu.edu/Projects/Library/Source/Branches/NASABot/Src/Math/SphereImpl.hpp $
...
Sphere3<T>::Sphere3( CONST Vector3<T>& P0, CONST Vector3<T>& P1, CONST Vector3<T>& P2, CONST Vector3<T>& P3 )
{
  ...
  if (Math::Abs(m11) < CONSTants<T>::ZeroTolerance)
    throw Math::MathException(this,"Attempted to construct a sphere from 4 co-planar points");

  m_Position[0] =  0.5 * m12 / m11;
  m_Position[1] = -0.5 * m13 / m11;
  m_Position[2] =  0.5 * m14 / m11;

  m_Radius = Math::Sqrt( m_Position[0]*m_Position[0] + m_Position[1]*m_Position[1] + m_Position[2]*m_Position[2] - m15/m11 );
}
//-----------------------------------------------------------------------------------------------------------------------------------------------------
template<class T>
...
T Sphere3<T>::Volume()
{
  return T(4)/T(3)*Math::CONSTants<T>::Pi*m_Radius*m_Radius*m_Radius;
}
//-----------------------------------------------------------------------------------------------------------------------------------------------------
template<class T>
T Sphere3<T>::Area()
{
  return T(4)*Math::CONSTants<T>::Pi*m_Radius*m_Radius;
}
//-----------------------------------------------------------------------------------------------------------------------------------------------------
