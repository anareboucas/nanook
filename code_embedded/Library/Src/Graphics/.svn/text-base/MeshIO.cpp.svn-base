/***************************************************************************************************
 * File:         $URL: http://obtuse.cs.jhu.edu/Projects/Library/Source/Branches/NASABot/Src/Graphics/MeshIO.cpp $
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
VOID Mesh::ImportBNPTS( LPCTSTR Filename, IProgress* pProgress )
{
  ...
  ReadPlyVerticesBinary(pStream, pProgress);
}
//----------------------------------------------------------------------------------------------------------------------------------------------------
VOID Mesh::ImportPLY( LPCTSTR Filename, IProgress* pProgress )
{
  ...
  while(!S.EndOfStream())
  {
    ...
    if (A[0] == "format")
    {
      ...
      if (A[2] != "1.0")
        throw InvalidFormatException(this, String::Format( TEXT("Ply version is %s.  Only version 1.0 is supported"), A[2].GetBuffer() ) );
    }
  ...
  for(SIZE_T i=0; i<Elements.Length(); i++)
  {
    ...
    if (Elements[i]->Name() == "face")
    {
      ...
      ReadPlyFacesBinary(pStream, pProgress);
    }
    ...
    if (Elements[i]->Name() == "triangle")
    {
      TriangleData.AddAttribute( TriangleAttribute::VERTICES );
      TriangleData.Resize(Elements[i]->Count());
      ReadPlyTrianglesBinary(pStream, pProgress);
    }
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
VOID Mesh::ReadPlyVerticesBinary( Ptr<Stream> pStream, IProgress* pProgress )
{
  if (pProgress)
    pProgress->BeginStep(TEXT("Reading Vertices"), 0, VertexData.GetCount());

  VertexData.Load(pStream, 0, VertexData.GetCount());

  if (pProgress)
    pProgress->EndStep();
}
//----------------------------------------------------------------------------------------------------------------------------------------------------
VOID Mesh::ReadPlyTrianglesBinary( Ptr<Stream> pStream, IProgress* pProgress )
{
  if (pProgress)
    pProgress->BeginStep(TEXT("Reading Triangles"), 0, TriangleData.GetCount());

  UINT Count = TriangleData.GetCount();
  TriangleData.MapView(PageAccess::READWRITE);
  LPBYTE pData = (LPBYTE)TriangleData.GetTableData();
  while(Count > 0)
  {
    if (pProgress)
      pProgress->UpdateStep(TriangleData.GetCount() - Count);
    UINT BlockSize = Min(Count, 65536u);
    pStream->Read(pData, BlockSize * TriangleData.GetElementSize());

    pData += BlockSize * TriangleData.GetElementSize();
    Count -= BlockSize;
  };
  TriangleData.Unmap();
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
VOID Mesh::ReadPlyFacesBinary( Ptr<Stream> pStream, IProgress* pProgress )
{
  if (pProgress)
    pProgress->BeginStep(TEXT("Reading Triangles"), 0, TriangleData.GetCount());

  struct TRIANGLE { UINT Vertices[3]; };
  ...
  {
    pStream->Read(&N, 1);
    pStream->Read(pData, sizeof(TRIANGLE));
    pData += sizeof(TRIANGLE);
  }
  TriangleData.Unmap();
...
#if 0
//----------------------------------------------------------------------------------------------------------------------------------------------------
VOID Mesh::WritePly( Ptr<Stream> pStream )
{
  TextStream S = TextStream(pStream);
...
/*
  ...
  S.WriteLine("comment Produced by Mesh::WritePly");
